...
function ModelCard(props) {
    ...
    if (props.status !== status) {
        ...
        setColor(props.status === "ready" ? "green" : "orange");
    }
    ...
    return (
        <div onMouseOver={displayAnomalies} onClick={deleteModel}
             className="model-card-container border rounded-pill" style={{backgroundColor: color}}>
            {/* add a tooltip when hover this card model */}
            <OverlayTrigger transition={false} data-bs-toggle="tooltip" data-bs-placement="top"
                            overlay={<Tooltip id={model.id}>{props.tooltip}</Tooltip>}>
